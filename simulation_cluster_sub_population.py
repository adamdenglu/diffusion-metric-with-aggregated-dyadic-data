...
def run_simulation(
    ...
    est_in, est_out, est, tau = simulation(
        graph, rep, prob, ratio, model, cluster=True
    )
    # save results
    path = f"./results/cluster_sub_population/{dataset}/"
...
if __name__ == "__main__":
    prob = 0.5
    # ratios = np.linspace(0.2, 1, 5)
    ratios = [0.05, 0.1, 0.25, 0.5, 1]
    rep = 100
    ...
    Parallel(n_jobs=-1, verbose=5)(
        ...
        for ratio in ratios for model in models for dataset in datasets
        for s in range(100)
    )
